...
echo "Compile and add CLK Config"
gcc -o minimal_clk minimal_clk.c
cp ./minimal_clk /usr/bin/
chmod u+s /usr/bin/minimal_clk

chmod +x gpclkconfig.sh
cp ./gpclkconfig.sh /etc/init.d/
update-rc.d gpclkconfig.sh defaults


#alsactl -d -f ./default.bkp store  wm8960soundcard
#alsactl -f ./test.bkp restore  wm8960soundcard
alsactl -f ./LineAlsa.sett restore  wm8960soundcard
